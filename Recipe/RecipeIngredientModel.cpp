...
QVariant RecipeIngredientModel::data(const QModelIndex &index, int role) const
{
    ...
    if(role == Qt::BackgroundColorRole && index.column() < columnCount()) {
        GrainIngredient *grainIngredient = qobject_cast<GrainIngredient *>(ingredient->ingredient());
        if(grainIngredient) {

            /* Get the color contribution of this ingredient, and display it's color value */
            double maltColorUnits = (ingredient->quantity().valueToPound() * grainIngredient->color())
                                    / _recipe->volume().valueToGallon();
            double standardReferenceMethod = 1.4922 * pow(maltColorUnits, 0.6859);

            switch((int)standardReferenceMethod * 10) {
            case 0:
